...
// creating new div tag which hold icons
...
function optionSelected(answer) {
    ...
    if(userAns == correcAns) {
        ...
        console.log("Your answer = " + userScore);
    } else {
...
function showResult() {
    ...
    if(userScore > 3 ) {
        let scoreTag = `<span> and congrats!, you got <p>${userScore} out of </p> ${questions.length}</span>`;
        scoreText.innerHTML = scoreTag;
    } else if (userScore > 1) {
        let scoreTag = `<span> and nice, You got <p>${userScore} out of ${questions.length}</p></span>`;
        scoreText.innerHTML = scoreTag;
    } else {
...
function queCounter(index) {
    let totalQueCountTag = `<span><p>${index} of ${questions.length}</p> Questions</span>`;
